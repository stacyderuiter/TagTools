...
fnum = ct(k,1) ;
kf = find(ct(:,1)==fnum) ;
csc = [0;cumsum(ct(kf,3))] ; % Finds all cue table entries for that file
...
fname = fn{fnum} ; %CHANGE BY FHJ from fname = fn{kf} ;
...
if len<n,
   if fnum==max(ct(:,1)),
      ...
      fname = fn{fnum+1} ; % CHANGE BY FHJ from fname = fn{kf+1} ;
