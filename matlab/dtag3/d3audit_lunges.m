function     RES = d3audit_lunges(tag, recdir, prefix, tcue,RES, fhigh, z)
%
%     RES = d3audit_Gg(tag, recdir,prefix,tcue,RES, fhigh, z)
%     Audit tool for dtag 3.
%     tag is the tag deployment string e.g., 'sw03_207a'
%     recdir is the path to the folder where wav files are stored
%     tcue is the time in seconds-since-tag-on to start displaying from
%     RES is an optional audit structure to edit or augment
%     fhigh is upper y-axis limit, in ***Hz***, for spectrogram.  Note that
%       fhigh does not affect acoustic sampling rate, only spectrogram
%       display.
%     z is logical: 1 to display depth profile, 0 to display only large
%       spectrogram.
%     Output:
%        RES is the audit structure made in the session. Use saveaudit
...
%       stacy deruiter, patricia arranz 2012-2014

%%%%%%%%%%%%%%%%%%% Set audit defaults %%%%%%%%%%%%%%%%%%%%%

NS = 10 ;          % number of seconds to display
BL = 1024 ;         % specgram (fft) block size
...
SOUND_FH = 100 ;     % high-pass filter for sound playback - 0 for no filter
...
AFS_RESAMPLE       = 96000 ; % resample  acoustic data to this afs to speed up 
DEFAULT_FHIGH      = AFS_RESAMPLE/2 ; % Default maximum frequency shown in spectrogram
temp_wav_location = 'C:\tempsound.wav'; %file path and filename for saving temporary sound clips
...
if nargin <3 || isempty(prefix)
    prefix = [tag(1:2), tag(6:9)] ; %short tag ID string, e.g., 'gg251a'
end
...
switch prefix(1:2),
   ...
   case 'pw',      % for pilot whale use:
      FH = 10000 ;       
      TC = 0.5e-3 ;           % power averaging time constant in seconds
   case 'gm',      % also for pilot whale use:
      FH = 10000 ;       
      TC = 0.5e-3 ;       
...
% If needed, lower sample rate before making filters
if fhigh*2 <= AFS_RESAMPLE
    if afs>AFS_RESAMPLE
        afs=AFS_RESAMPLE;
    end
end
...
while 1,
   [x,afs_org] = d3wavread(tcue+[0 NS],recdir,prefix) ;
    x = x(:,CH);
  ...
  % Construct spectrogram - use top line for Matlab version with specgram
   ...
   [B F T] = specgram(x,BL,afs,hamming(BL),floor(BL/1.3)) ;
   ...
   hold on, hhh = plot([0 0],0.8*fhigh/2000*[1 1],'k*-') ; hold off
  ...
  % Note: currently not used; cl is never plotted.
   if THRESH,
       [cl xx] = audit_findclicks(xx,thresh,fs) ;
   end
   ...
   while done == 0,
      ...
      elseif button=='a',
         try
            ...
            [x_temp,afs_org] = d3wavread(min(current)+[0 length_temp],...
      ...
      elseif button=='i',
            if all(current>0),
                ...
                [x_temp,afs_org] = d3wavread(min(current)+[0 length_temp],recdir, [tag(1:2) tag(6:9)], 'wav' ) ;
                x_temp = x_temp(:,CH);
                
                % Write a full-sample version to temporary file
                try
                    wavwrite(x_temp,afs_org,16,temp_wav_location);
                catch 
                    disp([' Error writing to ' temp_wav_location '; close other programs and try again \n or change file save location in d3audit_Gg at line 60.'])
                end
...
function   [cc,xx] = audit_findclicks(xx,thresh,fs)
%
...
%     August, 2000
%     modified June 2003
%     adjusted SDR June 2014 to use values calculated earlier in d3audit
