...
%        respect to each click. Optional. Default is [-0.001 0.025].
%     DD is the data structure defined below. DD can be passed as an
%        input argument to allow multiple work sessions. Optional (if empty
%        or not supplied new structure will be created).
%
...
%  majohnson@whoi.edu
%  February 2005
%  modifications sdr july 2013 (check compatibility with new d3 tools)
...
while next<2,
   while next==1,
      ...
      if ncl>3,
         ...
         drawnow ; 
         fprintf('Go to next block or quit (q=quit; anything else=go on)? ')
