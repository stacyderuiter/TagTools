function     RES = d3tagaudit(tag,tcue,RES,fhigh,upperplot,lowerplot)

%     Tagaudit tool adapted for version 3 DTAG deployments. Subject to change.
%
%     R = d3tagaudit(tag,tcue,R) will start the audit program for the
%     specified deployment tag at start cue tcue and using a pre-existing
%     audit structure R.
%
%     R = d3tagaudit(tag,tcue,R,fhigh,upperplot,lowerplot) will start a
...
%     upperplot is the data to be displayed in the top panel:
%           'envelope' for absolute envelope of signal energy (default)
%           'prh' for pitch, roll and heading data
%           NOTE: click detector option needs to be fixed.
%     lowerplot is the data to be displayed in the bottom panel:
%           'z' for depth (default)
%           'j' for jerk (requires high resolution sensor data)
%
%     Output:
...
if nargin<5 | isempty(upperplot)
    upperplot = 'envelope'; % Default plots amplitude envelope in top plot
elseif ~any(strcmp(upperplot,{'envelope','prh'}))
    disp('upper plot type not recognized')
    upperplot = 'envelope'; % Default plots amplitude envelope in top plot
end

if nargin<6 | isempty(lowerplot)
    lowerplot = 'z'; % Default plots depth in bottom plot
end


%%%%%%%%%%%%%%%%%%% Load prh file %%%%%%%%%%%%%%%%%%%%%

k = loadprh(tag) ;           % read p and fs from the sensor file
if k==0,
    fprintf('Unable to find a PRH file - continuing without\n') ;
    p = [] ; fs = [] ;
    lowerplot='z';
    if strcmp(upperplot,'prh'),
        upperplot='envelope';
    end
elseif ~exist('pitch')
    if strcmp(upperplot,'prh')
        disp(' PRH file does not include pitch-roll-heading data')
        upperplot='envelope';
    end    
end
...
while 1,
   ...
   [x,afs_org] = d3wavread([tcue tcue+NS],recdir, [tag(1:2) tag(6:9)], 'wav' ) ;
   x = x(:,CH);
   ...
   if strcmp(upperplot,'envelope')
       ...
       axis([tcue tcue+NS -30 -10]) ;
       ylabel('Intensity, dB')
   elseif strcmp(upperplot,'prh')
   ...
   if ~isempty(p)
       ...
       switch lowerplot
           ...
           case 'j' % NEED HIGH SAMPLE RATE SENSOR DATA
               if ~isempty(Aw),
