...
%**************************************************************************
% tag = 'pe12_134a'; %enter the tag ID string
% tagcal = dt3_105; %REALLY IT WAS 102 BUT NO CAL FILE AVAILABLE %calibration info for the tag used for this deployment
% hours = -10; %offset between GMT and local time. GMT + hours = local time at experiment site
% decl = 9.6326; %local declination angle - in degrees. 4.8 is approx for Sarasota. 
% %you can get precise decl info from NOAA: http://www.ngdc.noaa.gov/geomagmodels/Declination.jsp
% mindive = 50; %for prhpredictor -- tells it to use dives > mindive m to calibrate prh.
% %enter path to data directory
% %enter tag on time - [year month day hour minute second] NOW IN UTC TIME!!!
% tagontime = [2012 5 14 1 57 4]; %UTC for tag3
% %enter the tag on position [lat, lon] in decimal degrees (use +/- for E/W and N/S)
% position = [19.60329, -156.04873]; %
tag = 'gm12_139a'; %enter the tag ID string
tagcal = dt3_105; %calibration info for the tag used for this deployment
hours = -10; %offset between GMT and local time. GMT + hours = local time at experiment site
decl = 9.6165; %local declination angle - in degrees. 4.8 is approx for Sarasota. 
%you can get precise decl info from NOAA: http://www.ngdc.noaa.gov/geomagmodels/Declination.jsp
mindive = 50; %for prhpredictor -- tells it to use dives > mindive m to calibrate prh.
%enter path to data directory
%enter tag on time - [year month day hour minute second] NOW IN UTC TIME!!!
tagontime = [2012 5 18 20 23 41]; %UTC for tag3
%enter the tag on position [lat, lon] in decimal degrees (use +/- for E/W and N/S)
position = [19.47515, -156.06644]; %
% tag = 'gg11_269a';
% tagcal = dt3_106;
% hours = -7;
% mindive = 200;
