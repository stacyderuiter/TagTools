function    C = d3findclicks(recdir,prefix,cue,INOPTS,C)
%
%     C = d3findclicks(recdir,prefix,cue)
%     C = d3findclicks(recdir,prefix,cue,filt)
%     C = d3findclicks(recdir,prefix,cue,[],C)
%     C = d3findclicks(recdir,prefix,cue,filt,C)
%		Click sequence finder for stereo drecdir,prefixs
%     recdir,prefix is the recdir,prefix deployment string e.g., 'sw03_207a'
%     cue is the time in seconds-since-recdir,prefix-on to start working from.
...
%			filt is given or filt=[] the species prefix in recdir,prefix is used
...
%    1  click_cue in seconds since recdir,prefix on
%    2  angle of arrival with respect to recdir,prefix x-axis in degrees
%    3  peak level
%
%  mark johnson, WHOI
%  majohnson@whoi.edu
...
[x fs] = d3wavread(cue(1)+[0 0.1],recdir,prefix) ;
if OPTS.fh(2)>fs/2,
   OPTS.fh(2) = fs*0.46 ;
end
