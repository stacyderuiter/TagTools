function    CL = d3findmissedclicks(recdir,prefix,CL,FBP,TH,cue)
%
%    C = d3findmissedclicks(recdir,prefix,C,[FBP,TH,cue])
...
[x,afs] = d3wavread(CL(1,1)+[0 0.1],recdir,prefix) ;
...
while 1,
   ...
   x = d3wavread([st ed],recdir,prefix) ;
   if size(x,2)>1,
      xf = filter(b,a,x(:,2)+x(:,1)) ;
   else
      xf = filter(b,a,x) ;
   end
   xx = hilbenv(xf) ;
