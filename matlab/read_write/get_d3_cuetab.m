...
%		The file (called _'prefix''suffix'cues.mat) is saved in the same directory
%		as the data. If this file is deleted, it is automatically re-generated 
...
%        1. File number
%        2. Start time in seconds since the reference time
...
for k=1:length(fn),
   ...
   if isempty(blks),
      fprintf('\n Erro: no WAVBLK data or timing files - check version of d3read and re-run\n') ;
		return
	end
...
while 1,				% check the timing of the BLKS
   ...
   if terr(k)>0,
      ...
      if k<size(BLKS,1) && (BLKS(k,1)==BLKS(k+1,1)),
         fprintf(' => overrun in file %s of %3.3f seconds (%d samples)\n',...
               fn{BLKS(k,1)},-terr(k),-serr(k)) ;
      else
         fprintf(' => overrun between files %s and %s of %3.3f seconds (%d samples)\n',...
               fn{BLKS(k,1)},fn{BLKS(k+1,1)},-terr(k),-serr(k)) ;
      end
   end
end
