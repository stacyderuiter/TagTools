function		add_nc_smrt(fname,X, type)
...
%       type is the data type - 'double' by default, or may be 'char'
% 
%		Example:
%		 add_nc('dog17_124a',A)
...
if ~isempty(vname),		% X is a sensor structure
	if ~isfield(X,'data') || isempty(X.data),
		nccreate(fname,vname, 'Datatype', type);
   else
      if size(X.data,2)>1,
         nccreate(fname,vname,'Datatype', type, 'Dimensions',{[vname '_samples'],size(X.data,1),[vname '_axis'],size(X.data,2)});
      else
         nccreate(fname,vname, 'Datatype', type, 'Dimensions',{[vname '_samples'],size(X.data,1)});
      end
