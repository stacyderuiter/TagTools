...
kg = find(strcmp('GPS (raw) 2 [raw]',HDR)) ;
HDR = {HDR{[1:kg-1 kg+1:end]}} ;    % eliminate GPS time field (GPS (raw) 2) as it will be combined with GPS date (GPS (raw) 1)
nf = length(HDR)-1 ;
npartf = 0 ;
delete('_ttpart*.mat') ;
DN = [] ; X = [] ;
...
while 1,
    ...
    if ~isempty(s)
        ...
        for kk=1:length(kl)-1,    % for each line
            ...
            ke = find(diff(kc)==1)+1;        % find empty fields
            if ~isempty(ke)
                L = [L(1:kc(ke(1))-1), [78,97,78] , L(kc(ke(1))+1:end)] ;
            else
                dg = char(L(kc(kg(1)-1)+1:kc(kg(1)+1)-1)) ;
                ...
                L = [L(1:kc(kg(1)-1)), double(char(sprintf('%.10f',dgn))) , L(kc(kg(1)+1):end)] ;
            end    
