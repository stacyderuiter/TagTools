function    D=crosstool(tag2,cl1,cue,offset,D)
%
%     D=crosstool(rtag,scl,cue,offset,[D])
%     Collect distance between data from a pair of tagged whales.
%     rtag is the deployment name of the receiving whale.
...
if length(cue)==1,
   len = 20 ;                 % length of segment to analyze in seconds
else
   len = abs(diff(cue)) ;
   cue = min(cue) ;
end
...
while 1,
   ...
   while next==1,
       ...
       if next,
          ...
          x2 = tagwavread(tag2,cue+offset,len+intervl(2)) ;
