function     [RES,AXs,hhh] = ggtagaudit(tag,tcue,RES, fhigh)
%
...
%     fhigh is the highest frequency to display on the spectrogram in Hz-
%     default is 48 kHz
%     Output:
%        R is the audit structure made in the session. Use the m-file saveaudit
%        to save this to a file (type "help saveaudit" for more info).
%
%     OPERATION
...
%     - type 'i' to play the selected sound segment (set start and end
%     times by clicking with the mouse before typing "i").
...
switch tag(1:2),
   ...
   case 'sw',      % for sperm whale use:
      FH = 3000 ;       
    case 'zc',
        FH = 20000; %for ziphius use
    case 'gg',
        FH = 20000; %for grampus use
...
while 1,
   ...
   axes(AXs), imagesc(tcue+T,F,BB,CLIM) ;
   axis xy, grid ; 
   if ~isempty(p),
      set(AXs,'XTickLabel',[]) ;
   else
   ...
   while done == 0,
      ...
      elseif button=='p',
            chk = 1; %min(size(x,2),2) ;
            if ~isempty(bs),
               xf = filter(bs,as,x(:,chk)) ;
               sound(volume*xf,afs/SOUND_DF,16) ;
            else
               sound(volume*x(:,chk),afs/SOUND_DF,16) ;
            end
      elseif button=='i', %play sound clip (interval between two clicks) - from Frants H Jensen, 2012
            if all(current>0),
                length_temp=0.1*ceil(abs(diff(current)*10));
                [x_temp] = tagwavread(tag,min(current),length_temp);
                %wavwrite(x_temp(:,CH),afs,16,'C:\tempsound.wav');
      ...
      elseif button==1,
         if gy<0 | gx<tcue | gx>tcue+NS
            fprintf('Invalid click: commands are f b s l p x q i\n')
            ...
            set(hhh,'XData',current) ;
             ...
             hold on; plot(sort(current),[gy gy], 'k*-'); hold off
         end
      end
   end
end
