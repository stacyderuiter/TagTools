function    [DD,W] = echotool(tag,cue,CL,intervl,DD)
%
%     DD=echotool(tag,cue,CL,[intervl,DD])
...
c = round(c) ;
R = zeros(length(ki),length(c)) ;

if CH==0 & size(xf,2)>1,
   for k=1:length(c),
      R(:,k) = sum(xf(c(k)+ki,:)')'/size(xf,2) ;
   end
else
   for k=1:length(c),
      R(:,k) = xf(c(k)+ki,CH) ;
