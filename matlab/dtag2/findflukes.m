function    [v,ph,mx,fr] = findflukes(Aw,Mw,fs,FR,TH,T)
%
...
%    second
%    ph is a band pass filtered version of the pitch data, from which the
%    fluke strokes are detected.
%
%    mark johnson, WHOI
%    majohnson@whoi.edu
...
[b a] = butter(2,FR/4/(fs/2), 'low') ;   % make a low-pass filter (added 'low' -- matlab help does NOT specify it's the default. sdr)
...
for k=1:nsamps,
   ...
   if ~isempty(kk),
      y(k) = mean(fr(kk)) ;  %I think this has zeros in it...makes things go haywire...sdr 2012
   end
end
...
% fr = interp1(tv,fr,(1:length(Aw))./fs);
