function    [M,CAL,fs,Mz] = acousondeAutoCalMag(X,CAL,test,mindepth)
...
%will not have +ve and -ve!
km = find(strcmp(type,'mag')) ;
%kb = find(strcmp(type,'mag.bridge')) ;
kt = find(strcmp(type,'tempr'));
kp = find(strcmp(type,'press'));
...
% and return
if isempty(test),
   [M,fs,Mz] = full_rate_cal(X,CAL,km) ;
   return
end
...
[M,fs,Mz] = full_rate_cal(X,CAL,km,kt,kp) ;
return


function    [M,fsm,Mz] = full_rate_cal(X,CAL,km,kt, kp)
%
