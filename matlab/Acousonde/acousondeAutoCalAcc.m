function    [A,CAL,fs] = acousondeAutoCalAcc(X,CAL,test,mindepth,ch)

%    [A,CAL,fs] = acousondeAutoCalAcc(X,CAL,test,mindepth,ch)
...
kp = find(strcmp(cal,'press')) ;        %find the pressure channel in X
...
[A,fs] = full_rate_cal(X,CAL,C,ka,kt, kp) ;
return


function    [A,fs] = full_rate_cal(X,CAL,C,ka,kt,kp)
%
