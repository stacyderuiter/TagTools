function     [RES,AXs,hhh] = cee_tagaudit(tag,tcue,tjump, RES)
%     modified version of dtag audit to mark controlled exposure times in a
%     dtag record
%     Stacy DeRuiter, August 2009
%
...
%     tjump is the time between consecutive exposures
%     RES is an optional audit structure to edit or augment
%     Output:
%        R is the audit structure made in the session.
%
%     OPERATION
%     Type or click on the display for the following functions:
%     - type 'f' to go to the time window of the next exposure (assuming
%       constant spacing)
%     - type 'b' to go to the previous exposure (assuming constant spacing)
...
%     - type 'n' to jump to time of next exposure.
%     - type 'r' to jump to time of previous exposure.
...
BL = 4096*2 ;         % specgram (fft) block size
...
volume = 100 ;      % amplification factor for audio output - often needed to
...
%             plotRES(AXc,RES,[tcue tcue+NS]) ;
      ...
      elseif button =='n'
            cc = sort(current) ;
            lastmfa = max(cc(1),tcue);
            tcue = lastmfa + floor(tjump)-0.5 ;
            done = 1;
            
      elseif button =='r'
            cc = sort(current) ;
            lastmfa = min(cc(1),tcue);
            tcue = lastmfa - floor(tjump)-0.5 ;
            done = 1;
      ...
      elseif button==1,
         if gy<0 | gx<tcue | gx>tcue+NS
            fprintf('Invalid click: commands are f b s l p x n r q\n')
