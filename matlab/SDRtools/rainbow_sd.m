function    [cl,a,m,q, ckdur]=rainbow_sd(x,xr,fs,INOPTS)
%
...
if ~isempty(OPTS.fh),
   ...
   if length(OPTS.fh)>1,
      ...
      [br ar] = butter(6,OPTS.f_reject/(fs/2));
   else
      [bf af] = butter(6,OPTS.fh/(fs/2),'high');
      [br ar] = butter(6,OPTS.f_reject/(fs/2),'high');
   end
   xr = filter(br,ar,x);
   x = filter(bf,af,x);
...
[cl,clend,levl] = getclickx_sd(xx,xxr,thr,fs,OPTS);
...
a = real(180/pi*asin(cm*OPTS.soundspeed/OPTS.separation/fs)) ;
m = max(abs(H1))';
k = find(q>OPTS.qthr);
