...
defopts = struct('fh',10e3,'fl',2000,'blanking',0.01,'fr',[0,0],...
          'protocol','first','ffull',0,'win',0.01, 'dur_lims', [0.001, 0.005]) ;
...
if ~isfield(opts,'env') || opts.env ~= 1 ;
    disp('I am doing the filtering hahaha....')
   if ~isempty(fh),
      if length(fh)==1 || fh(2)==0,
         [b a] = butter(4,fh(1)/(fs/2),'high') ;
         [br ar] = butter(4,f_reject(1)/(fs/2),'high') ;
      else
         [b a] = butter(4,fh/(fs/2)) ;
         [br ar] = butter(4,f_reject/(fs/2)) ;
      end
      % high-pass or band-pass filter signal before detection
      xf = filter(b,a,x);
      % high-pass or band-pass filter for comparison band (reject putative
      % signals with energy in this band)
      xr = filter(br,ar,x);
   end
   ...
   if isempty(fl),
      ...
      [b_env,a_env] = butter(4,fl/(fs/2)) ;
      % compute rms envelope
      xf = sqrt(abs(filtfilt(b_env,a_env,abs(xf).^2))) ;
      xr = sqrt(abs(filtfilt(b_env,a_env,abs(xr).^2))) ;
   end
else
    xf = x; %if input x is an envelope rename it xf
end

% xf and xr are now envelopes (f for where we want to detect clicks, r for
% where we want to reject clicks)
...
if thresh>0,
    if isempty(xr)
        ...
        dxx = diff(xf>thresh & xr<(thresh/3)) ;
    end
   cc = find(dxx>0)+1 ;
end
...
for k=1:length(cc),%-1, %NOTE: SDR removed the "-1" Aug 2015.  Seems like if you leave that in, then the reported duration of the last click in the segment will always last until the end of the clip??
...
switch protocol,
   case 'first',
         cplus = 0*cc ;
         for k=1:length(cc),
            cplus(k) = find(X(:,k)>thresh, 'first') ;
            cplus_end(k) = find(X(:,k)>thresh, 'last');
            level(k) = X(cplus(k),k) ;
         end
   case 'max',
         [level cplus] = max(X) ;
   ...
   case 'max-10',
         ...
         for k=1:length(cc),
            ...
            cplus_end(k) = max(find(X(:,k)>0.3*level(k))) ;
         end
   case 'max-6',
         level = max(X) ;
         for k=1:length(cc),
            cplus(k) = min(find(X(:,k)>0.5*level(k))) ;
            cplus_end(k) = max(find(X(:,k)>0.5*level(k))) ;
         end
   case 'steepest',
...
level = level(:) ;
clend = (cc+cplus_end(:)+T(1)-2)/fs ;

%eliminiate pulses that are the wrong duration
cdur = clend - cl ;
keep = find(cdur >= min(dur_lims) & cdur < max(dur_lims));
cl = cl(keep);
clend = clend(keep);


if length(cplus)>1 && nargout>3,
   mincplus = min(cplus) ;
