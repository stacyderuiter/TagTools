...
#' 
#' @param X, Y, etc, Lists or vectors/matrices of time series data.
#' @param fsx, fsy, etc, The sampling rates in Hz for each data object. Sampling rates are not needed when the data object is a list.
...
#' @return h is a list of vectors of handles to the lines plotted. There is a list of handles for each axis.
...
>>>>>>> 5b51228cd39992c6fa5f756beda95ca05c763985
  ...
  for (k in 1:length(X)) { #now we are ready to plot
    axis(ax[k]) 
    h[[k]] <- plot(((1:nrow(X[[k]])) / fsrt[k, 1] + fsrt[k, 3]) * (1 / ddiv), X[[k]])
    set(ax[k], 'XLim', xlims)
    if (fsrt[k, 2] == 1) {
