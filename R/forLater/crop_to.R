...
#' 
#' Possible input combinations: crop_to(X, tcues) if X is a sensor structure, crop_to(X, sampling_rate, tcues) if x is a vector or matrix.
...
#'          d <- find_dives(P,300)
#'          P2 <- crop_to(P, c(d$start[2], d$end[2]))	#crop to 2nd dive
#'          plott(P2)
#'          #plot shows the dive profile and acceleration of the second dive

crop_to <- function(X, sampling_rate, tcues) {
