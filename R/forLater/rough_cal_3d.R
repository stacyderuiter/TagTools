...
#' @param fsrt The expected field strength at the measurement location in the same units as X
...
rough_cal_3d <- function(X, fstr) {
  ...
  lims <- matrix(c(quantile(x[, 1], c(0.1, 100 - 0.1) / 100), quantile(x[, 2],c(0.1, 100 - 0.1) / 100), quantile(x[, 3], c(0.1, 100 - 0.1) / 100)), nrow = 2)
  g <- 2 * fstr / diff(lims)
  offs <- -mean(lims) * g
  G <- list()
