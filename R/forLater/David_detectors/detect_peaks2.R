...
#' 
#' @param data A vector (of all positive values) or matrix of data to be used in peak detection.
...
#' @param fs The sampling rate in Hz of the date. This is used to calculate the bktime in the case that the input for bktime is missing.
...
#    dots <- as.list(substitute(list(...)))
  ...
  if (plot_peaks == TRUE) {
    ...
    y <- peaks$peak_max
    par(new = TRUE)
    plot(x, y, pch = 9, type = "p", col = "orange", xlim = c(0, nrow(data)), ylim = c(0, max(dnew)), cex = .75)
    abline(a = thresh, b = 0, col = "red", lty=2)
    pts <- graphics::locator(n = 3)
    if (length(pts$x) == 3) {
      thresh <- pts$y[1]
      bktime <- max(pts$x[2:3]) - min(pts$x[2:3])
      peaks <- detect_peaks(dnew, fs, thresh, bktime, plot_peaks = FALSE)
    } else {
      if (length(pts$x) == 1) {
        thresh <- pts$y[1]
        peaks <- detect_peaks(dnew, fs, thresh = thresh, plot_peaks = FALSE)
      } else {
        if (length(pts$x) == 2) {
          bktime <- max(pts$x) - min(pts$x)
          peaks <- detect_peaks(dnew, fs, bktime = bktime, plot_peaks = FALSE)
        } else {
          peaks <- detect_peaks(dnew, fs, thresh, bktime, plot_peaks = FALSE)
        }
      }
