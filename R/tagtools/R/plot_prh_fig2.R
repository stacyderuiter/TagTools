#' Plot second detail (one-dive) plot for prh_predictor
#'
...
#' @param seg information about which segments of data to plot. A one-row matrix with 5 columns: start end end times of first box, and start and end times of second box; then an indicator that is -1 for descent and 1 for ascent.
...
plot_prh_fig2 <- function(A, sampling_rate, seg, prh, prompt) {
  ...
  # x limits
  xl2 <- c(min(seg[1:4], na.rm = TRUE) - 30, max(seg[1:4], na.rm = TRUE) + 30)
  ...
  graphics::legend("bottomright",
    bty = "n", legend = names(hex_cols),
    text.col = hex_cols, horiz = TRUE
  )
  # add boxes
  graphics::rect(
    xleft = seg[c(1, 3)], xright = seg[c(2, 4)],
