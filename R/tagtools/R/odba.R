...
#' @param sampling_rate The sampling rate in Hz of the acceleration signals. 
...
#' @note When hoping to use the default (FIR filtering) method to calculate odba, use the inputs A, sampling_rate, and fh. When hoping to use the "vedba" or "wilson" method, use the inputs A, n, and method.
#' @export
#' @example 
#' BW <- beaked_whale
...
odba <- function(A, sampling_rate, fh = NULL, method = "fir", n = NULL) {
  if (nargs() < 3) {
    stop("Three inputs are required")
  }
  if (method == "fir") {
    if (missing(sampling_rate) | is.null(fh)) {
      stop("sampling_rate and fh are required inputs to compute odba by the FIR method")
    }
