...
Use `load_nc()` to read `testset5.nc`, which is from a pilot whale in the Canary Islands. Write it to the object `sw` to follow along with the code in this vignette. If you need to download this dataset, it is available from the animal tags website, either at http://animaltags.org/ or https://animaltags.netlify.app/ . 
...
*If you'd like to continue working through these vignettes, consider `complementary-filtering`.* 
