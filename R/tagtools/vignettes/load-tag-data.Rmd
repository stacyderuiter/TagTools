...
For this example, you will load in the data file from a CATs tag.The data file is already stored in a .nc file called 'cats_test_raw.nc'. The data is available on the animal tags website: http://animaltags.org/ or https://animaltags.netlify.app/ . The function we will be using to achieve this is called `load_nc()`.
...
Another refresher -- if you get an error saying the function doesn't exist, make sure you have `tagtools` in your library! It won't be if you've restarted R recently. Running `library(tagtools)` does the trick as long as you've installed the package in the past. 

All right, finally time to get out `load_nc`! Here we go:
...
*If you'd like to continue working through these practicals, `plots-and-cropping` would be one good choice. In it, you'll get to start visualizing some data of the types you've been learning to load.* 