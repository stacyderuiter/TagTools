...
Welcome to the plot and cropping vignette! On behalf of the team behind tagtools, thanks for taking some time to get to know our package. We hope it makes your life a little easier. 
...
If you've already completed `load-tag-data` within the same R session, you can skip down a bit to [Exploratory data plots]. If not, you're just a couple of lines behind. For this example, we will make use of a CATS tag data file that is already stored in a .nc file called 'cats_test_raw.nc'. This dataset is built into the `tagtools` package, so you can access it using `system.file`. 
