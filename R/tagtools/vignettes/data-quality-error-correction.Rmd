...
Welcome to the `data-quality-error-correction` vignette! Thanks for taking some time to get to know our package; we hope it is useful to you.
...
## Data quality checks
For this practical we will use data from a tag attached to the back of a sperm whale. The data is in: `rawtestset3.nc`. 

(As usual: Make sure that `rawtestset3.nc` is in your working directory. Is `load_nc()` not in the mood to exist? Then `library(tagtools)` is your friend. How about if `testset6.nc` decides to actually be `no such file or directory`? Well, search for it in your file finder, or if you can't find it, you can download it from animaltags.org. After that, `getwd()` and `setwd("PathToWorkingDirectoryInQuotes")` are your friends.)  

```{r, echo = TRUE, eval = FALSE}
sw <- load_nc('rawtestset3')
```
...
*Note that the code above assumes that the file is saved in your current working directory. `getwd()` will quickly show you what your current working directory is. You may need to use `setwd("yourPathInQuotes")` to change the working directory, or alternatively specify a full or relative path for the nc file.*
...
Inspect the depth, acceleration and magnetometer data:
...
The magnetometer data should also have fairly constant vector magnitude equal to the field strength where the data came from. It should be about 48 $\mu$T in this case since the tag was deployed at $28.48^{\circ}$, $-89.052^{\circ}$ latitude/longitude. 
...
```{r, eval = FALSE, echo = TRUE}
fs <- sw$A$sampling_rate # get the sampling rate of A and M for plotting
# plott fstr, and incl (converting from radians to degrees)
plott(X = list(fstr = AMcheck$fstr, incl = AMcheck$incl*180/pi), fsx = fs)
```
...
The top plot shows the field strength of `A` and `M`. These should be close to 9.8 and 48, respectively, with little variation. If not, the acceleration or magnetometer data need re-calibration. Usually, the problem is with `M` and is due to stray magnetic fields in the tag. Fix this by doing a 'hard-iron correction', which the function `fix_offset_3d` can do. Again, recall that `M` is stored within `sw` as `sw$M`, and write the result as an object `Mf`. Then look at what `Mf` contains.
...
plott(X = list(fstr = AMcheck2$fstr, incl = AMcheck2$incl*180/pi), fsx = fs)
...
```{r, echo = FALSE, eval = TRUE}
axis_table <- tibble::tibble(`Sensor axis` = c('x', 'y', 'z'),
                         `Tag axis` = c('-x', 'z', 'y'))
...
plott(X = list(fstr = AMcheck3$fstr, incl = AMcheck3$incl*180/pi), fsx = fs)
```
...
*If you'd like to continue working through these vignettes, you now have a couple of different options for how to estimate and use animal orientation. Select one or both as best fits your data or interests.*

**Pitch, roll, and Heading: Inferring seal behaviour from orientation**

*Estimating pitch roll and heading angles and using them to make ethograms. This vignette will use tag data that is aligned with the animal's body axes (this could be either because the tag was glued in place or because the orientation of the tag on the animal has already been corrected).*

**Tag orientation, From tag to sperm whale frame**

*Estimating and correcting the orientation of a tag on an animal. This vignette will use tag data that are not aligned to the animal's body axes, e.g., because the tag was applied to a free-moving whale.*
