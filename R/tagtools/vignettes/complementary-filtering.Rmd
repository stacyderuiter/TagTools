...
The magnetometer is not sensitive to specific acceleration, so why do stroking motions show up in `Mseg`? Refer to your plots of `Mseg` and `Dseg` to try and figure out which axis the stroking motions show up in. 
...
*The black line is altitude, because it, along with depth, trends gradually upwards over the course of the seven minutes. So most of the movement is in this vertical axis.*
</div>

Any idea what kind of movement it might represent?

<button class="btn btn-primary" data-toggle="collapse" data-target="#ans_kind_of_movement"> _Show/Hide Answer_ </button>
<div id="ans_kind_of_movement" class="collapse">
*Our whale moving towards the surface?*
</div>
...
```{r, echo = TRUE, eval = FALSE}
plot.window(xlim = c(0, 2), ylim = c(-4, 4))
...
```{r, echo = FALSE, eval = TRUE}
yl <- c(0, 4)
plott(X = list(Ah1 = Ah1, Ah2 = Ah2), fsx = sampling_rate, xl = c(0, 1), yl = yl)
...
plott(X = list(HfAccel4 = Ah5), fsx = sampling_rate, interactive = TRUE)
```
...
plott(X = list(HfAccel4 = Ah5), fsx = sampling_rate, interactive = FALSE)
```
