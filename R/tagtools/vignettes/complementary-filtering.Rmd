...
The sampling rate of these is the same as for the original data. For simplicity, make a variable `fs` equal to the sampling rate and use `plott()` to plot the two filtered accelerations along with the dive profile:
...
plott(X = list(`Depth (m)` = bw$P$data,
               `LF Accel` = Alow, 
               `HF Accel` = Ahigh),
      fsx = fs, xlim = c(35*60, 42*60)/3600)
```
...
Ah1 <- crop_to(Ahigh, sampling_rate = fs, tcues = intvl1)
Ah2 <- crop_to(Ahigh, sampling_rate = fs, tcues = intvl2)
```
...
*If you'd like to continue working through these vignettes, `jerk-transients` is a very logical choice. So is `more-filtering`.*
