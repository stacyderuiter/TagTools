...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; The resulting data shows the lunge feeding events detected by the detect_peaks function while using an "optimal" threshold and blanking time. As you have seen, this method does not return perfect results. This is due to a variety of reasons, one of which is the intended generalizability of the function to allow for the detection of any peak from a signal of data with values greater than or equal to zero given a few parameters. I have shown you a very specific application of this tool to determine the times of lunge feeding events from multisensor data, but their are seemingly infinite possible applications for this function.