...
Load the test dataset mn12\_186a. This dataset has already been converted from the source file that was offloaded from the tag into a NetCDF file. In doing so, some metadata was gleaned from the file and added to the data. Other metadata was added by hand. Use load_nc to load a NetCDF file:
...
```{r, eval=FALSE}
load_nc('/Users/Sebastian/tagtools/testdata/mn12\_186a\_raw')
```
...
This displays an interactive depth plot. Follow the instructions to select the obvious diving section of the data and then click finish. The function returns a new data structure which contains just the selected part of the dive profile. 

The resulting sensor data list shalso contains fields that document what you just did. They should look like:
